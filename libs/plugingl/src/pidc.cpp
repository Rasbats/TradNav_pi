 ...
 *
 * Project:  OpenCPN
 * Purpose:  Layer to perform wxDC tradnavawing using wxDC or opengl
...
#ifdef ocpnUSE_GL
/* tradnavaw a half circle using triangles */
...
void piDrawGLThickLine( float x1, float y1, float x2, float y2, wxPen pen, bool b_hiqual )
{
    ...
    //    The first is assumed tradnavawn, second is assumed space
    wxDash *dashes;
    ...
    if( n_dashes ) {
        ...
        float xa = x1;
        float ya = y1;
        float ltradnavaw = t1 * dashes[0];
        float lspace = t1 * dashes[1];

        while( lrun < lpix ) {
            //    Dash
            float xb = xa + ltradnavaw * cosf( angle );
            float yb = ya + ltradnavaw * sinf( angle );

            if( ( lrun + ltradnavaw ) >= lpix )         // last segment is partial tradnavaw
            {
                xb = x2;
                yb = y2;
            }
        ...
        /* wx tradnavaws a nice rounded end in dc mode, so replicate
    ...
    if( dc )
        ...
        if( b_hiqual ) {
            ...
            if( pen_width > 1 ) {
                GLint parms[2];
                glGetIntegerv( GL_ALIASED_LINE_WIDTH_RANGE, &parms[0] );
                if( pen_width > parms[1] ) b_tradnavaw_thick = true;
                    else
                        glLineWidth( pen_width );
            } else
                glLineWidth( pen_width );
...
//             glUniformMatrix4fv( matloc, 1, GL_FALSE, (const GLfloat*)cc1->GetpVP()->vp_transform);
            ...
            if( n_dashes ) {
                ...
                ltradnavaw = wxMax(ltradnavaw, 4.0);
                lspace = wxMax(lspace, 4.0);
                lpix = wxMin(lpix, 2000.0);

                while( lrun < lpix ) {
                    //    Dash
                    float xb = xa + ltradnavaw * cosa;
                    float yb = ya + ltradnavaw * sina;
                    ...
                    xa = xa + ( lspace + ltradnavaw ) * cosa;
                    ya = ya + ( lspace + ltradnavaw ) * sina;
                    lrun += lspace + ltradnavaw;
                       ...
                       wxCoord yoffset, wxPen pen, bool b_hiqual )
{
    ...
    /* for dashed case, for now just tradnavaw thick lines */
    wxDash *dashes;
    ...
    /* nicer than than rendering each segment separately, this is because thick
       line segments tradnavawn as rectangles which have different angles have
    ...
    if( dc )
...
#ifdef __WXQT__
        SetGLAttrs( false );            // Some QT platforms (Antradnavoid) have trouble with GL_BLEND / GL_LINE_SMOOTH
#else
        SetGLAttrs( b_hiqual );
#endif
        bool b_tradnavaw_thick = false;
        ...
        if( b_hiqual ) {
            ...
            if( m_pen.GetWidth() > 1 ) {
                GLint parms[2];
                glGetIntegerv( GL_ALIASED_LINE_WIDTH_RANGE, &parms[0] );
                if( m_pen.GetWidth() > parms[1] ) b_tradnavaw_thick = true;
                else
                    glLineWidth( wxMax(GLMinSymbolLineWidth, m_pen.GetWidth()) );
            } else
                glLineWidth( wxMax(GLMinSymbolLineWidth, 1) );
...
/* tradnavaw the arc along corners */
static void tradnavawrrhelper( wxCoord x0, wxCoord y0, wxCoord r, int quatradnavant, int steps )
{
#ifdef ocpnUSE_GL
#ifndef USE_ANTradNavOID_GLES2
    float step = 1.0/steps, rs = 2.0*r*step, rss = rs*step, x, y, dx, dy, ddx, ddy;
    switch(quatradnavant) {
...
void piDC::tradnavawrrhelperGLES2( wxCoord x0, wxCoord y0, wxCoord r, int quatradnavant, int steps )
{
#ifdef ocpnUSE_GL
    float step = 1.0/steps, rs = 2.0*r*step, rss = rs*step, x, y, dx, dy, ddx, ddy;
    switch(quatradnavant) {
...
void piDC::DrawRoundedRectangle( wxCoord x, wxCoord y, wxCoord w, wxCoord h, wxCoord r )
{
    if( dc )
        ...
        tradnavawrrhelperGLES2( x2, y1, r, 0, steps );
        tradnavawrrhelperGLES2( x1, y1, r, 1, steps );
        tradnavawrrhelperGLES2( x1, y2, r, 2, steps );
        tradnavawrrhelperGLES2( x2, y2, r, 3, steps );
        ...
        // Perform the actual tradnavawing.
        ...
        if( ConfigureBrush() ) {
            ...
            tradnavawrrhelper( x2, y1, r, 0, steps );
            tradnavawrrhelper( x1, y1, r, 1, steps );
            tradnavawrrhelper( x1, y2, r, 2, steps );
            tradnavawrrhelper( x2, y2, r, 3, steps );
            glEnd();
        }
    ...
    if( dc )
        ...
        if( usemask ) {
            ...
            if( !image.GetOrFindMaskColour( &mr, &mg, &mb ) && !a ){
                printf("trying to use mask to tradnavaw a bitmap without alpha or mask\n" );
            }
...
//                        e[off * 4 + 3] = ( ( r == mr ) && ( g == mg ) && ( b == mb ) ? 0 : 255 );
            ...
            glRasterPos2i( x, y );
            glPixelZoom( 1, -1 ); /* tradnavaw data from top to bottom */
            if(image.GetData())
