...
TDNavObjectChanges::TDNavObjectChanges() : pugi::xml_document()
{
    //ctor
    m_bFirstPath = true;
    m_TDchanges_file = 0;
...
TDNavObjectChanges::TDNavObjectChanges(wxString file_name) : pugi::xml_document()
{
    //ctor
    m_ODfilename = file_name;
    m_TDchanges_file = fopen(m_ODfilename.mb_str(), "a");
    m_bFirstPath = true;
    m_ptODPointList = new ODPointList;
}

TDNavObjectChanges::~TDNavObjectChanges()
{
    //dtor
    if(m_TDchanges_file)
        fclose(m_TDchanges_file);
...
void TDNavObjectChanges::RemoveChangesFile( void )
{
    if(m_TDchanges_file)
        fclose(m_TDchanges_file);
...
void TDNavObjectChanges::CreateRotatingNavObjChangesBackup()
{
...
bool TDNavObjectChanges::GPXCreateODPoint( pugi::xml_node node, ODPoint *pop, unsigned int flags )
{
    wxString s;
    pugi::xml_node child;
...
bool TDNavObjectChanges::GPXCreatePath( pugi::xml_node node, ODPath *pInPath )
{
    ODPath *pPath;
...
bool TDNavObjectChanges::AddPath( ODPath *pb, const char *action )
{
    if( !m_TDchanges_file ) return false;
    ...
    pugi::xml_writer_file writer(m_TDchanges_file);
    object.print(writer, " ");
#ifdef __WXMSW__
    fclose(m_TDchanges_file);
    m_TDchanges_file = fopen(m_ODfilename.mb_str(), "a");
#else
    fflush(m_TDchanges_file);
#endif

    return true;
}

bool TDNavObjectChanges::AddODPoint( ODPoint *pOP, const char *action )
{
    SetRootGPXNode();
...
bool TDNavObjectChanges::AddGPXPathsList( PathList *pPaths )
{
    SetRootGPXNode();
...
bool TDNavObjectChanges::AddGPXPath(ODPath *pPath)
{
    SetRootGPXNode();
    if ( m_bFirstPath ) {
        m_bFirstPath = false;
...
bool TDNavObjectChanges::AddGPXODPoint(ODPoint *pWP )
{
    SetRootGPXNode();
...
bool TDNavObjectChanges::AddGPXODPointsList( ODPointList *pODPoints )
{
    SetRootGPXNode();
...
bool TDNavObjectChanges::CreateNavObjGPXPoints( void )
{
...
bool TDNavObjectChanges::CreateNavObjGPXPaths( void )
{
...
bool TDNavObjectChanges::CreateAllGPXObjects()
{
    SetRootGPXNode();
...
void TDNavObjectChanges::SetRootGPXNode(void)
{
...
bool TDNavObjectChanges::LoadAllGPXObjects( bool b_full_viz )
{
...
ODPoint * TDNavObjectChanges::GPXLoadODPoint1( pugi::xml_node &opt_node,
...
ODPath *TDNavObjectChanges::GPXLoadPath1( pugi::xml_node &odpoint_node  , bool b_fullviz,
                    bool b_layer,
                    bool b_layerviz,
...
ODPoint *TDNavObjectChanges::ODPointExists( const wxString& name, double lat, double lon )
{
    wxODPointListNode *node = g_pODPointMan->GetODPointList()->GetFirst();
    while( node ) {
...
ODPoint *TDNavObjectChanges::ODPointExists( const wxString& guid )
{
    wxODPointListNode *node = g_pODPointMan->GetODPointList()->GetFirst();
    while( node ) {
        ODPoint *podp = node->GetData();
...
ODPoint *TDNavObjectChanges::tempODPointExists( const wxString& guid )
{
...
void TDNavObjectChanges::tempODPointRemove( const wxString& guid )
{
    wxODPointListNode *node = m_ptODPointList->GetFirst();
    while( node ) {
...
void TDNavObjectChanges::InsertPathA( ODPath *pTentPath )
{
    if( !pTentPath )
        return;
...
ODPath *TDNavObjectChanges::PathExists( const wxString& guid )
{
...
ODPath *TDNavObjectChanges::PathExists( ODPath * pTentPath )
{
    wxPathListNode *path_node = g_pPathList->GetFirst();
    while( path_node ) {
        ODPath *ppath = path_node->GetData();
...
bool TDNavObjectChanges::SaveFile( const wxString filename )
{
    save_file(filename.fn_str(), "  ");
    return true;
}

bool TDNavObjectChanges::ApplyChanges(void)
{
...
int TDNavObjectChanges::LoadAllGPXObjectsAsLayer(int layer_id, bool b_layerviz)
{
    if(!g_pODPointMan)
        return 0;
...
void TDNavObjectChanges::UpdatePathA( ODPath *pPathUpdate )
{
...
int TDNavObjectChanges::Load_CSV_File(wxString FileName, int layer_id, bool b_layerviz)
{
